...
const requestWithdrawal = async (userId, data) => {
  ...
  try {
    ...
    if (parseFloat(vendor.saldo_saat_ini) < jumlahTarik) {
      throw new Error(
        `Saldo tidak mencukupi. Saldo Anda: Rp ${vendor.saldo_saat_ini}`,
      );
    }

    // 1. Kurangi saldo vendor
    await vendor.update(
      { saldo_saat_ini: parseFloat(vendor.saldo_saat_ini) - jumlahTarik },
      { transaction },
    );
    ...
    await AuditLog.create(
      {
        ...
        data_lama: JSON.stringify({
          saldo_awal: parseFloat(vendor.saldo_saat_ini) + jumlahTarik,
        }),
  ...
  try {
    ...
    if (status === "rejected") {
      await withdrawal.vendor.update(
        {
          saldo_saat_ini:
            parseFloat(withdrawal.vendor.saldo_saat_ini) +
            parseFloat(withdrawal.jumlah),
        },
        { transaction },
      );
    }
    ...
    await withdrawal.update(
      { status, bukti_transfer_url: bukti_transfer_url || null },
      { transaction },
    );
